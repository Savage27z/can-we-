...
def _run_one(job: tuple) -> dict:
    ...
    strategy, instrument, exit_name, cost_name, slippage = job
    try:
...
def summary_row(instrument: str, summary: dict) -> dict:
    return {"instrument": instrument, **summary}


def format_table(rows: list[dict]) -> str:
    header = (f"{'pair':8} {'n':>4} {'W/L':>7} {'win%':>6} {'grossR':>7} {'netR':>7} "
    ...
    for r in rows:
        ...
        lines.append(f"{r['instrument']:8} {r['signals']:>4} {wl:>7} "
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    jobs = [(args.strategy, p, args.exit, args.costs, args.slippage_pips) for p in pairs]
    print(f"strategy {args.strategy}  exit {args.exit}  costs {args.costs}"
          ...
          + f"  data {config.DATA_DIR}  {len(pairs)} instrument(s)\n")
    ...
    rows = [summary_row(o["instrument"], scoring.summarize(o["trades"], o["months"])) for o in done]
    ...
    if args.out_dir:
        ...
        record = {
            ...
            "costs": args.costs, "slippage_pips": args.slippage_pips,
