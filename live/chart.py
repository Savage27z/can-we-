...
CANDLES_SHOWN = 80
RIGHT_PADDING_SLOTS = 15   # empty space right of the last candle, for price labels
...
def render_chart(candles: pd.DataFrame, state: LiveState) -> bytes:
    ...
    for s in setups:
        ...
        ys.append(s.sweep_extreme - span * 0.09 if s.direction == "bullish"
                  else s.sweep_extreme + span * 0.09)
    ys += setup_levels
    ...
    buy_levels = [v for v in state.liquidity_buy_side
                  if near[0] <= v <= near[1] and not_a_setup_level(v)]
    ...
    fig = Figure(figsize=(12.8, 7.2), dpi=100, facecolor=BG)
    ax = fig.add_axes([0.04, 0.085, 0.885, 0.80])  # right margin holds the price ticks
    ax.set_facecolor(BG)
    ...
    ax.vlines(x, lows, highs, colors=colors, linewidth=1.0, zorder=3)
    body = np.maximum(np.abs(closes - opens), (y_hi - y_lo) * 0.0006)
    ax.bar(x, body, bottom=np.minimum(opens, closes), width=0.64, color=colors, zorder=4)
    ...
    # Setups
    for setup in setups:
        _draw_setup(ax, setup, times, n, line_end, right_edge, fmt, labels, y_hi - y_lo)
    ...
    ax.text(1.004, state.current_price, format(state.current_price, fmt), color="white",
            fontsize=8.5, fontweight="bold", va="center", ha="left", zorder=6, clip_on=False,
            transform=ax.get_yaxis_transform(),
            bbox={"facecolor": price_color, "edgecolor": "none", "pad": 2.5})

    for y, text, color in _spread_labels(labels, (y_hi - y_lo) * 0.03):
        ax.text(label_x, y, text, color=color, fontsize=8, va="center", ha="left",
                zorder=5, clip_on=False)

    # Header
    pair_label = state.pair.replace("_", "")
    fig.text(0.04, 0.95, f"{pair_label}  ·  H4", color=TEXT, fontsize=15, fontweight="bold",
             ha="left", va="center")
    fig.text(0.04, 0.915, f"{_status_line(state)}   ·   as of {state.as_of[:16].replace('T', ' ')} UTC",
             color=MUTED, fontsize=9.5, ha="left", va="center")
    bias = state.daily_bias
    fig.text(0.97, 0.95, f" DAILY BIAS: {bias.upper()} ", color="white", fontsize=10,
    ...
    if state.news.status == "blackout" and state.news.blackout_events:
        ...
        fig.text(0.97, 0.905, f"NEWS BLACKOUT: {event.currency} {event.title} ({event.when})",
                 color="#f85149", fontsize=9, fontweight="bold", ha="right", va="center")
    fig.text(0.97, 0.02, "Structural analysis, not financial advice", color=MUTED,
...
def _draw_setup(ax, setup: ActiveSetup, times: pd.Series, n: int, line_end: float,
                right_edge: float, fmt: str, labels: list, y_span: float) -> None:
    ...
    if sweep_pos is not None:
        ...
        ax.text(sweep_pos, y - offset * 0.9 if bullish else y + offset * 0.9, "Sweep",
                color=SWEEP, fontsize=8.5, fontweight="bold", ha="center",
    ...
    if setup.fvg_low is not None and setup.fvg_high is not None:
        ...
        ax.add_patch(Rectangle((x0 - 0.4, setup.fvg_low), line_end - x0 + 0.4,
                               ...
                               edgecolor=fill, alpha=0.22, linewidth=0.8, zorder=1))
        ax.text(x0 - 0.2, setup.fvg_high, " FVG", color=fill, fontsize=8, fontweight="bold",
    ...
    if setup.status == "pending_confirmation" and setup.confirmation_level is not None:
        ax.hlines(setup.confirmation_level, fvg_pos if fvg_pos is not None else start, line_end,
                  colors="white", linestyles=":", linewidth=1.1, zorder=3)
    ...
    if setup.status == "live_trade" and setup.entry_price is not None:
        ...
        if setup.stop_price is not None:
            ...
            ax.hlines(setup.stop_price, x0, line_end, colors=DOWN, linewidth=1.3, zorder=3)
        ...
        if setup.target_price is not None:
            ax.add_patch(Rectangle((x0, min(setup.entry_price, setup.target_price)), line_end - x0,
                                   ...
                                   alpha=0.12, linewidth=0, zorder=1))
            ax.hlines(setup.target_price, x0, line_end, colors=UP, linewidth=1.3, zorder=3)
            reward = f" (+{setup.rr:.2f}R)" if setup.rr is not None else ""
            labels.append((setup.target_price,
                           f"Target {format(setup.target_price, fmt)}{reward}", UP))
        ax.hlines(setup.entry_price, x0, line_end, colors=TEXT, linewidth=1.3, zorder=3)
